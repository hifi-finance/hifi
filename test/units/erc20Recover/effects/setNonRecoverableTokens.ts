  ...
  describe("when the caller is the administrator", function () {
    describe("when the contract was not initialized", function () {
      describe("when the token array is non-empty", function () {
        describe("when the tokens are compliant", function () {
          it("sets the non-recoverable tokens", async function () {
            const collateralAddress: string = this.stubs.collateral.address;
            await this.contracts.erc20Recover.connect(this.signers.admin).setNonRecoverableTokens([collateralAddress]);
          ...
          it("emits a SetNonRecoverableTokens event", async function () {
            const collateralAddress: string = this.stubs.collateral.address;
            await expect(
              this.contracts.erc20Recover.connect(this.signers.admin).setNonRecoverableTokens([collateralAddress]),
            )
              .to.emit(this.contracts.erc20Recover, "SetNonRecoverableTokens")
              .withArgs(this.accounts.admin, [collateralAddress]);
          });
        });
      ...
      describe("when the token array is empty", function () {
        it("reverts", async function () {
          await expect(
            this.contracts.erc20Recover.connect(this.signers.admin).setNonRecoverableTokens([]),
    ...
    describe("when the contract was initialized", function () {
      ...
      it("reverts", async function () {
        await expect(
          this.contracts.erc20Recover.connect(this.signers.admin).setNonRecoverableTokens([AddressZero]),
