...
export default function shouldBehaveLikeSetCollateralizationRatio(): void {
  /* Equivalent to 110% */
  const newLiquidationIncentiveMantissa: BigNumber = OnePercentMantissa.mul(110);
    ...
    describe("when the liquidation incentive is not valid", function () {
      describe("when the liquidation incentive is higher than 150%", function () {
        it("reverts", async function () {
          const overflowLiquidationIncentiveMantissa: BigNumber = OnePercentMantissa.mul(150).add(1);
          await expect(
            this.contracts.fintroller
              .connect(this.signers.admin)
      ...
      describe("when the liquidation incentive is lower than 100%", function () {
        it("reverts", async function () {
          const underflowLiquidationIncentiveMantissa: BigNumber = OnePercentMantissa.mul(100).sub(1);
          await expect(
            this.contracts.fintroller
              .connect(this.signers.admin)
