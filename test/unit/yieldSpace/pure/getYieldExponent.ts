...
import { CUTOFF_TTM, G1, G2, SCALE, ZERO } from "../../../../helpers/constants";
...
export default function shouldBehaveLikeGetYieldExponent(): void {
  ...
  context("when not too far from maturity", function () {
    const g1TestSets = [
      ...
      [secondsInYears(1).mul(SCALE), G1, fp("0.7624999999982368")],
      [CUTOFF_TTM.mul(SCALE), G1, fp("0.097500007524375436")],
    ];
    const g2TestSets = [
      [ZERO, G2, fp("1")],
      [sfp("1e-18"), G2, fp("1")],
      ...
      [secondsInYears(1).mul(SCALE), G2, fp("0.736842105261204211")],
      [CUTOFF_TTM.mul(SCALE), G2, sfp("8.337258101e-9")],
    ];
    const testSets = g1TestSets.concat(g2TestSets);
