...
export default function shouldBehaveLikeListBond(): void {
  ...
  context("when the caller is the owner", function () {
    ...
    it("emits a ListBond event", async function () {
      await expect(this.contracts.fintroller.connect(this.signers.owner).listBond(this.mocks.hTokens[0].address))
        .to.emit(this.contracts.fintroller, "ListBond")
        .withArgs(this.signers.owner.address, this.mocks.hTokens[0].address);
    });
  });
}
