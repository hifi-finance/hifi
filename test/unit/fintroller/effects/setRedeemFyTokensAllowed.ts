...
export default function shouldBehaveLikeSetRedeemFyTokensAllowed(): void {
    ...
    describe("when the bond is listed", function () {
      ...
      it("sets the value to true", async function () {
        await this.contracts.fintroller
          .connect(this.signers.admin)
          .setRedeemFyTokensAllowed(this.stubs.fyToken.address, true);
        const newState: boolean = await this.contracts.fintroller.getRedeemFyTokensAllowed(this.stubs.fyToken.address);
      ...
      it("sets the value to false", async function () {
        await this.contracts.fintroller
          .connect(this.signers.admin)
          .setRedeemFyTokensAllowed(this.stubs.fyToken.address, false);
        const newState: boolean = await this.contracts.fintroller.getRedeemFyTokensAllowed(this.stubs.fyToken.address);
        expect(newState).to.equal(false);
      });

      it("emits a SetRedeemFyTokensAllowed event", async function () {
        await expect(
          this.contracts.fintroller
            .connect(this.signers.admin)
            .setRedeemFyTokensAllowed(this.stubs.fyToken.address, true),
