...
export function integrationTestHifiPool(): void {
  describe("HifiPool", function () {
    beforeEach(async function () {
      ...
      await this.contracts.hToken.__godMode_mint(this.signers.alice.address, MAX_UD60x18);
      await this.contracts.underlying.__godMode_mint(this.signers.alice.address, MAX_UD60x18);

      // Approve the pool to spend an infinite amount of hTokens and underlying from Alice's wallet.
      await this.contracts.hToken.connect(this.signers.alice).approve(this.contracts.hifiPool.address, MAX_UD60x18);
      await this.contracts.underlying.connect(this.signers.alice).approve(this.contracts.hifiPool.address, MAX_UD60x18);
    });
