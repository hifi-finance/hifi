...
export function shouldBehaveLikeConstructor(): void {
  ...
  context("when provided pool address is zero", async function () {
    it("reverts", async function () {
      ...
      await expect(deployUniswapV3PriceFeedPromise).to.be.revertedWith(UniswapV3PriceFeedErrors.ZERO_ADDRESS_POOL);
    });
  });
